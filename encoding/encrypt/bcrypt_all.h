//
// Created by SinSayChen on 2024/11/1.
//
...
#define F(s, x) ((((s)[(((x) >> 24) & 0xFF)] + (s)[0x100 + (((x) >> 16) & 0xFF)]) ^ (s)[0x200 + (((x) >> 8) & 0xFF)]) + (s)[0x300 + ((x) & 0xFF)])
...
void Blowfish_initstate(blf_ctx* c)
{
    ...
    static const blf_ctx initstate = { { { 0xd1310ba6, 0x98dfb5ac, 0x2ffd72db, 0xd01adfb7,
...
void Blowfish_expandstate(blf_ctx* c, const u_int8_t* data, u_int16_t databytes,
...
//
// extern "C" {
//    void GenerateHash(const char *input, int rounds, char *output) {
//        std::string data = Crypto::GenerateHash(input, rounds);
//        memcpy(output, data.c_str(), data.size());
//        //        return data.c_str();
//    }
//
//    int ValidatePassword(const char *password, const char *hash) {
//        bool ok = Crypto::ValidatePassword(password, hash);
//        return ok ? 1 : 0;
//    }
//}

void GenerateHash(const char* password, unsigned int size, unsigned int rounds, char* output)
{
    char salt[_SALT_LEN];
    ...
    node_bcrypt(password, size, salt, output);
}

int ValidatePassword(const char* password, unsigned int size, const char* hash)
{
    char got[61];
    node_bcrypt(password, size, hash, &got[0]);
    fprintf(stderr, "password %s -> got: %s\n", hash, got);
    return strncmp(hash, got, 60);
}
